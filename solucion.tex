...
\aux{cantVecinosVivos}{t : \toroide, f : \ent, c : \ent}{\ent}{\\
    (\sum_{i=f-1}^{f+1} (\sum_{j=c-1}^{c+1} if {(i \neq f \land j \neq c \land (t[i mod |t|][j mod |t[0]|]))} \ then \ 1 \ else \ 0 \ fi))}
\vspace{5mm}
...
\begin{proc}{seleccionNatural}{in ts: $\TLista{\toroide}$, out res: $\ent$}{}
    \pre{|ts| > 0 \wedge (\forall i : \ent)(0 \leq i < |ts| \implicaLuego esValido(ts[i]) \wedge (\exists k:\ent)(k > 0 \wedge muerteEnTicks (ts[i], k)) }
    \post{
...
\begin{proc}{enCrecimiento}{in t: $\toroide$, out res: $\bool$}{}
    ...
    \post{res = \True \Iff (\exists tEvo:toroide) \\
        ((|tEvo| = |t| \wedge |tEvo[0]| = |t[0]|) \yLuego evolucionToroide(t, tEvo) \\
