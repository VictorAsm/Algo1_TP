...
\begin{proc}{seleccionNatural}{in ts: $\TLista{\toroide}$, out res: $\ent$}{}
    ...
    \post{
        ...
        (\exists k,w : \ent)(k>w \wedge k>0 \wedge w>0 \wedge \\
            muerteEnTicks(ts[res],k) \ muerteEnTicks(ts[i], w)))
    }
...
\begin{proc}{fusionar}{in t1: $\toroide$, in t2: $\toroide$, out res: $\toroide$}{}
    ...
    \post{|result| = |t1| \yLuego |result[0]| = |t[0]|
    \\ \implica
...
\pred{posValida}{t: \toroide, pos: $\ent$x$\ent$}{
    if((0 \leq pos_0 \leq |t|) \wedge (0 \leq pos_1 \leq |t[0]|))
    then
        \True
    else
        \False
...
\pred{supervivenciaPosicion}{t : \toroide, posicion : \ent x\ent}{\\if (cantVecinosVivos(t, posicion_0, posicion_1) == \ 2 \vee \\ cantVecinosVivos(t, posicion_0, posicion_1) == 3) \ then \ \True \ else \ \False \ fi}
\vspace{5mm}

\pred{resurreccionPosicion}{t : \toroide, posicion : \ent x\ent}{\\if \ (cantVecinosVivos(t, posicion_0, posicion_1) == 3) \ then \ \True \ else \ \False \ fi}
\vspace{5mm}

\pred{sinRepetidos}{vivas: $\TLista{$\ent$ x $\ent$}$}{\\
    (\paraTodo i 0 |vivas| \implicaLuego (\paraTodo j 0 |vivas| \yLuego i \neq j \implica vivas[i] \neq vivas[j]))
}
\vspace{5mm}

\pred{sinPosicionesFuera}{t: \toroide, vivas: $\TLista{$\ent$ x $\ent$}$}{\\
    (\paraTodo i 0 |vivas| \implicaLuego (\existe j 0 |t| \implicaLuego (\existe k 0 |t[j]| \implicaLuego vivas[i]_0 = j \wedge vivas[i]_1 = k)))
}
\vspace{20mm}

\pred{esKesimaEvolucion}{t:toroide, k:\ent, result: toroide}{\\
    (\exists \ ts:seq<toroide>) \\
        (|ts|=k \ \yLuego \ ts[0] = t \wedge ts[k]=result \
        \wedge 
        \ (\forall i : \ent) \\
...
\pred{contieneToroideVivo}{result:toroide, t:toroide}{\\
    (\forall i:\ent)(0\leq i < |t| \yLuego (\forall j:\ent)(0\leq j < |t[i]| \implicaLuego (result[i][j] = \True \wedge t[i][j] = \True)
    ))
}
\vspace{5mm}
